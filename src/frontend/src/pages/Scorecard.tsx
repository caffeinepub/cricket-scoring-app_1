...
function computeBattingStats(deliveries: Delivery[], players: Player[]) {
  ...
  for (const d of deliveries) {
    ...
    if (!stats[id])
      stats[id] = { runs: 0, balls: 0, fours: 0, sixes: 0, dismissed: false };
    if (!d.isWide) {
      stats[id].balls++;
  ...
  return players
    ...
    .map((p) => ({
      ...
      ...stats[p.id.toString()],
      strikeRate:
        stats[p.id.toString()].balls > 0
          ? (
              (stats[p.id.toString()].runs / stats[p.id.toString()].balls) *
              100
            ).toFixed(1)
          : "0.0",
    }));
...
function computeBowlingStats(deliveries: Delivery[], players: Player[]) {
  ...
  for (const d of deliveries) {
    ...
    if (!stats[id])
      stats[id] = { runs: 0, balls: 0, wickets: 0, wides: 0, noBalls: 0 };
    stats[id].runs += Number(d.runs);
      ...
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-base">
            Innings {inningsNum} — {battingTeamName}
          </CardTitle>
        </CardHeader>
        <CardContent>
        ...
        <div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {bowlingStats.map(
                  ({ player, overs: o, runs: r, wickets: w, economy }) => (
                    <tr
                      key={player.id.toString()}
                      className="border-b border-muted/30"
                    >
...
export default function Scorecard() {
  ...
  const rawMatchId =
    params.matchId ?? localStorage.getItem("currentMatchId") ?? "";
  ...
  const isLoading =
    matchLoading || teamsLoading || innings1Loading || innings2Loading;

  // Aggregate error state
  const hasError =
    !isLoading && (matchError || teamsError || innings1Error || innings2Error);
  const firstError =
    matchErrorObj ?? teamsErrorObj ?? innings1ErrorObj ?? innings2ErrorObj;
    ...
    return (
      <div className="pb-24 px-4 pt-4 max-w-lg mx-auto space-y-4">
        ...
        <p className="text-muted-foreground">
          No match selected. Please start or resume a match first.
        </p>
  ...
  if (!match) {
    ...
    return (
      <div className="pb-24 px-4 pt-4 max-w-lg mx-auto space-y-4">
        ...
        <p className="text-muted-foreground">
          Match data not found. It may have been reset.
        </p>
        <Button variant="outline" onClick={() => navigate({ to: "/" })}>
          Go Home
        </Button>
  ...
  const innings1BattingTeam = allTeams.find(
    (t) => t.id === innings1Data?.battingTeamId,
  );
  const innings1BowlingTeam = allTeams.find(
    (t) => t.id === innings1Data?.bowlingTeamId,
  );
  const innings2BattingTeam = allTeams.find(
    (t) => t.id === innings2Data?.battingTeamId,
  );
  const innings2BowlingTeam = allTeams.find(
    (t) => t.id === innings2Data?.bowlingTeamId,
  );
