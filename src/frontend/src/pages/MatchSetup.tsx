...
export default function MatchSetup() {
  ...
  const tossWinnerTeam =
    teamList.find((t) => t.id.toString() === tossWinnerId) ?? null;
      ...
      <Card>
        <CardContent className="py-8 text-center">
          ...
          <p className="text-sm text-muted-foreground/70 mt-1">
            You need at least 2 teams to set up a match. Go to Teams to add
            more.
          </p>
        </CardContent>
      </Card>
...
function TossStep({
  ...
  const winnerTeam =
    teams.find((t) => t.id.toString() === tossWinnerId) ?? null;
...
function ConfirmStep({
  ...
  const battingFirst =
    tossChoice === TossChoice.Bat ? tossWinnerTeam : tossLoserTeam;
  const bowlingFirst =
    tossChoice === TossChoice.Bat ? tossLoserTeam : tossWinnerTeam;
