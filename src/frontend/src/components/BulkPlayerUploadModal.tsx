...
function validateAndParsePlayers(
  ...
  const existingNames = new Set(
    currentPlayers.map((p) => p.name.toLowerCase()),
  );
  ...
  return lines.map((name) => {
    ...
    if (name.length > 50) {
      return {
        name,
        status: "too-long",
        reason: "Name must not exceed 50 characters",
      };
    }
    if (!name.includes(" ")) {
      return {
        name,
        status: "no-space",
        reason: "Name must contain at least one space",
      };
    }
      ...
      >
        <DialogHeader>
          ...
          <DialogDescription style={{ color: "oklch(0.5 0.03 240)" }}>
            {step === "input" &&
              "Paste player names or upload a .txt/.csv file. One name per line or comma-separated."}
            {step === "preview" &&
              "Review the parsed names before adding them to the team."}
            {step === "result" &&
              "Bulk upload complete. See the summary below."}
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-4">
            ...
            >
              ...
              <ul className="list-disc list-inside space-y-0.5 text-xs">
                <li>
                  Each name must have at least one space (e.g., "John Smith")
                </li>
          ...
          <div className="space-y-3">
            ...
            {validCount === 0 && (
              <Alert variant="destructive">
                <AlertDescription>
                  No valid player names found. Please check the format
          ...
          {step === "preview" && (
            <>
              ...
              <Button
                ...
                style={{
                  background:
                    validCount > 0 ? "oklch(0.65 0.18 45)" : undefined,
