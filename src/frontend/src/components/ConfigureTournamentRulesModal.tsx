...
export default function ConfigureTournamentRulesModal({
  ...
  const numField = (label: string, field: keyof TournamentRules, min = 0) => (
          ...
          <div className="space-y-6 py-2">
            ...
            <section className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                {numField(
                  "Team Readiness Penalty (min)",
                  "teamReadinessPenaltyMinutes",
                )}
                {numField(
                  "Team Readiness Penalty (overs)",
                  "teamReadinessPenaltyOvers",
                )}
                {numField(
                  "Slow Over Rate Penalty (runs)",
                  "slowOverRatePenaltyRuns",
                )}
                {numField("Innings Duration (min)", "inningsDurationMinutes")}
                {numField("Default Penalty Runs", "defaultPenaltyRuns")}
                {numField(
                  "Wides/No-Ball Bowler Change Threshold",
                  "widesNoBallBowlerChangeThreshold",
                )}
              </div>
            </section>
            ...
            <section className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                {numField(
                  "Max Balls (Short Format)",
                  "maxBallsPerBatsmanShortFormat",
                )}
                {numField(
                  "Max Balls (Long Format)",
                  "maxBallsPerBatsmanLongFormat",
                )}
              </div>
            </section>
            ...
            <section className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                {numField(
                  "Max Overs (Short Format)",
                  "maxOversBowlerShortFormat",
                )}
                {numField(
                  "Max Overs (Long Format)",
                  "maxOversBowlerLongFormat",
                )}
              </div>
            </section>
