...
export default function OverTimeline({
  ...
  const overs: (Delivery & { _deliveryIndex: number })[][] = [];
  let legalCount = 0;
  ...
  for (let di = 0; di < deliveries.length; di++) {
    ...
    overs[overIndex].push({ ...d, _deliveryIndex: di });
    ...
    <Card>
      <CardContent className="pt-3 pb-3 space-y-2">
        ...
        {displayOvers.map((overDeliveries, idx) => {
          ...
          const legalInOver = overDeliveries.filter(
            (d) => !d.isWide && !d.isNoBall,
          ).length;
          const overRuns = overDeliveries.reduce((sum, d) => {
            return (
              sum + Number(d.runs) + (d.isWide ? 1 : 0) + (d.isNoBall ? 1 : 0)
            );
          }, 0);
          ...
          return (
            <div key={overNum} className="flex items-center gap-2">
              ...
              <div className="flex gap-1 flex-wrap flex-1">
                ...
                {Array.from({ length: Math.max(0, 6 - legalInOver) }).map(
                  (_, slotOffset) => {
                    const slotKey = `empty-${overNum}-ball${legalInOver + slotOffset}`;
                    return (
                      <div
                        key={slotKey}
