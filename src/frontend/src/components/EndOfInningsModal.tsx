...
interface EndOfInningsModalProps {
  isOpen: boolean;
  onClose: () => void;
  /** 1 = setting up innings 1 (fresh match start), 2 = setting up innings 2 (after innings 1 ends) */
...
export default function EndOfInningsModal({
  ...
  // inningsNumber 2 means innings 1 just ended and we are setting up innings 2
  const isSettingUpInnings2 = inningsNumber === 2;
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>
            {isSettingUpInnings2
              ? "End of Innings 1 — Set Up Innings 2"
              : "Set Up Innings 1"}
          </DialogTitle>
          <DialogDescription>
            {isSettingUpInnings2
              ? `Innings 1 complete. Score: ${score.runs}/${score.wickets}. Select openers and bowler for the second innings.`
              : "Select the opening batsmen and opening bowler to start the first innings."}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-2">
          ...
          >
            {isSettingUpInnings2 ? "Start Innings 2" : "Start Innings 1"}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
