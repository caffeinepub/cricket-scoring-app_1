    ...
    <div className="space-y-2">
      ...
      <div className="flex items-center justify-between px-1">
        ...
        <Badge
          ...
          className={
            isComplete ? "bg-[oklch(0.55_0.18_145)] text-white border-0" : ""
          }
        >
        ...
        <div className="p-2 space-y-1">
          {players.length === 0 ? (
            ...
            players.map((player) => {
              ...
              const checkboxId = `player-${String(player.id)}`;
              return (
                <label
                ...
                >
                  <Checkbox
                    ...
                    className={
                      isSelected
                        ? "border-[oklch(0.65_0.18_45)] data-[state=checked]:bg-[oklch(0.65_0.18_45)]"
                        : ""
                    }
                  />
                  <div className="flex-1 min-w-0">
                    <p
                      className={`text-sm font-medium truncate ${isSelected ? "text-foreground" : "text-foreground"}`}
                    >
                      {player.name}
                    </p>
                    <p className="text-xs text-muted-foreground">
                      {player.isBowler ? "Bowler" : "Batsman"} · #
                      {Number(player.battingOrder)}
                    </p>
                  </div>
                  {isSelected && (
                    <span className="text-[oklch(0.65_0.18_45)] text-xs font-bold">
                      ✓
                    </span>
                  )}
                </label>
