...
export function useMatchState(matchId: string | null) {
  ...
  const setMatchState = useCallback(
    (
      updater:
        | MatchStateData
        | ((prev: MatchStateData | null) => MatchStateData | null),
    ) => {
  ...
  const advanceBall = useCallback(
    (isLegal: boolean, isNoBall: boolean) => {
      setMatchState((prev) => {
        ...
        const newLegal = isLegal
          ? prev.legalBallsInOver + 1
          : prev.legalBallsInOver;
  ...
  /**
   * Transitions to innings 2 in a "pending setup" state:
   * sets currentInnings=2 but clears player IDs so isSetupComplete=false,
   * which will trigger the innings 2 setup modal.
   */
  const beginInnings2Setup = useCallback(() => {
    setMatchState((prev) => {
      if (!prev) return prev;
      const next: MatchStateData = {
        ...prev,
